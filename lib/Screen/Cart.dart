...
class StateCart extends State<Cart> with TickerProviderStateMixin {
  ...
  Widget listItem(int index, List<SectionModel> cartList) {
    ...
    double price = double.parse(
     ...
     finalPrice = newCartData[index]['final_price'];
    if (price == 0)
      price = double.parse(
                        ...
                        children: <Widget>[
                          ...
                          cartList[index]
                              ...
                              //  CUR_CURRENCY! + " " + price.toString()
                                    : CUR_CURRENCY! + " " + finalPrice.toString(),
                                // double.parse(cartList[index]
                                //             .productList![0]
                                //             .prVarientList![selectedPos]
                                //             .disPrice!) !=
  ...
  Widget cartItem(int index, List<SectionModel> cartList) {
    int selectedPos = 0;
    for(var i=0;i<cartList[0].productList!.length ;i++){
      print("final price check ${cartList[0].productList![i].finalPrice}");
    }
    ...
    if (price == 0)
      ...
      finalPrice = newCartData[index]['final_price'];
    print("ddddddddddddddd{$finalPrice}");
  ...
  Future<void> _getCart(String save) async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        print(" this is my param ^^ ${parameter.toString()}");
        ...
        if (!error) {
          print(msg);
          print("varient check ${getdata['variant_id']}");
          varienIdList = getdata["variant_id"];
          vId = varienIdList!.join(',');
          // varientIdList.add(getdata["variant_id"]);
          print("oooooo ${varienIdList.toString()} and @@ $vId");
          // varientList = getdata['variant_id'];
          var data = getdata["data"];
          newCartData = getdata["data"];
          print("ssssssssssssssssssssssss$getdata['delivery_charge']");
          dCharge = double.parse(getdata['delivery_charge']);
          setState(() {
          ...
          print("charges here ${deliveryCharge} and ${dCharge}");
          totalPrice = delCharge + oriPrice + dCharge;
          ...
          for (int i = 0; i < cartList.length; i++){
             products.add( newCartData[i]['final_price']);
             productPrice = products.join(',');
          }
          print("this is reuslt** ${productPrice}");
...
print(parameter.toString());
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (!error) {
            ...
            oriPrice = double.parse(data['sub_total']);
            deliveryCharge = double.parse(data['delivery_charges']);
            _controller[index].text = qty;
            totalPrice = 0;
                ...
                children: <Widget>[
                  ...
                  Container(
                    child: Column(mainAxisSize: MainAxisSize.min, children: <
                      ...
                      SimBtn(
                          size: 0.9,
                          title: status == "0"? "Send Request" : "Checkout",
                          onBtnSelected: () async {
                            if (status == "0") {
                              _sendRequest();
                            } else {
  ...
  Future<void> placeOrder(String? tranId) async {
    ...
    if (_isNetworkAvail) {
      ...
      List<SectionModel> cartList = context.read<CartProvider>().cartList;
      varientId = cartList[0].varientId;
      print("this is variantId @@ $varientId");
      for (SectionModel sec in cartList) {
        ...
        print("this is variantId @@ $varientId");
        // varientIdList.add(varientId.toString());
      }
      ...
      try {
        var parameter = {
          ...
          //varienIdList,
          QUANTITY: quantity,
          "temp_total": oriPrice.toString(),
          "delivery_charge_cart":totalPrice.toString(),
          // FINAL_TOTAL: totalPrice.toString(),
          "delivery_charge_p":dCharge.toString(),
          // DEL_CHARGE: deliveryCharge.toString(),
  ...
  print("DAAAAAAAAAAAAAAAAAAAAAAAAAaaaa==${headers.toString()}");
        Response response =
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (!error) {
            ...
            promoC.clear();
            var data = getdata["data"];
            print("Shivani+++++++++++++++++++++++${delCharge}");
            print("ShivaniVerma+++++++++++++++++++++++${totalPrice}");
            totalPrice = double.parse(data["final_total"]) + delCharge;
